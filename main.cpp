...
class UncopyableTestClass    {
public:
    UncopyableTestClass()
        : UncopyableTestClass(5) {
    }

    UncopyableTestClass(size_t num)
        : x_(num) {
    }

    UncopyableTestClass(const UncopyableTestClass& other) = delete;

    UncopyableTestClass& operator=(const UncopyableTestClass& other) = delete;

    UncopyableTestClass(UncopyableTestClass&& other) {
        x_ = exchange(other.x_, 0);
    }

    UncopyableTestClass& operator=(UncopyableTestClass&& other) {
        x_ = exchange(other.x_, 0);
        return *this;
    }
...
void TestNoncopiableMoveConstructor() {
    ...
    SimpleVector<UncopyableTestClass> vector_to_move;
    for (size_t i = 0; i < size; ++i) {
        vector_to_move.PushBack(UncopyableTestClass(i));
    }
    
    SimpleVector<UncopyableTestClass> moved_vector = move(vector_to_move);
    assert(moved_vector.GetSize() == size);
    assert(vector_to_move.GetSize() == 0);
...
void TestNoncopiablePushBack() {
    ...
    SimpleVector<UncopyableTestClass> v;
    for (size_t i = 0; i < size; ++i) {
        v.PushBack(UncopyableTestClass(i));
    }
...
void TestNoncopiableInsert() {
    ...
    // begin
    v.Insert(v.begin(), UncopyableTestClass(size + 1));
    assert(v.GetSize() == size + 1);
    assert(v.begin()->GetX() == size + 1);
    // end
    v.Insert(v.end(), UncopyableTestClass(size + 2));
    assert(v.GetSize() == size + 2);
    assert((v.end() - 1)->GetX() == size + 2);
    // middle
    v.Insert(v.begin() + 3, UncopyableTestClass(size + 3));
